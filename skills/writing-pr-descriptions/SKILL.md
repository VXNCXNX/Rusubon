...
description: >-
  ...
  Use ALWAYS before writing or editing a PR description, before `gh pr create`
  or `gh pr edit --body`, and when asked to improve an existing description.
  Puts the effect a person sees in the first line and the mechanism under it,
  routes each remaining fact to the form that carries it fastest, cuts
  everything a reviewer does not need, then holds what survives to a checkable
  shape. Ends with a scan test and a line check. Not for commit messages.
---

# Writing PR descriptions

Adapted from PostHog `.agents/skills/writing-pr-descriptions/SKILL.md` (MIT).
...
Get the order right first. Never buy shape at the cost of it.

Work in five passes: lead, route, cut, shape, check. Run all five.

Public PRs. Never put Slack quotes, customer names, customer data, secrets, or internal operational metrics in the body.
...
- Line 1 of Problem says what is different for a person, and who that person is. Name the surface they were on. Four shapes cover almost every PR:
  ...
  - A feature: what someone could not do, and now can.
...
> - A query issued while the app mounts is dropped and rejects 30 s later with "Canvas request timed out".
...
Attach before-and-after images on the PR. Assets on a public PR are public forever. Never upload customer data, customer names, secrets, Slack quotes, or internal info.
...
Keep diagrams simple. A syntax error renders as an error block. Pick `TD` for tall pipelines, `LR` for wide paths. Pair every `fill` with a text `color` so nodes stay legible in GitHub light and dark. Do not use a house color palette.
...
- State what you did not check. "Not run: the database-backed suites, because this machine has no database" is the most credible line in most bodies and the cheapest to write.
...
- The reason your design beats the obvious alternative belongs in Changes. A reviewer needs it to review.
...
> When the job's artifact glob matches one artifact (every selective-mode run), `download-artifact` extracts it flat, so spans get `job_key ...:None` and re-run recovery joins miss.
...
- The subject of a sentence is the change, not its author. Never "I", "me" or "my".
  "The exporter now retries once", never "I made the exporter retry once".
  An agent writing as "I" hands the assignee an account of work they did not do.
  Authorship is one stated fact in the Agent context section, not a voice the body speaks in.
...
18. Does the body name a customer, incident, Slack quote, or operational metric? Public PRs must not. Delete it.

## Agent context

Every PR body includes an **Agent context** section. List autonomy, tools, skills invoked (always include `writing-pr-descriptions` when this skill ran), and what changed across the session. One stated fact. Not a voice the rest of the body speaks in.
...
Review time goes to understanding the change rather than to finding defects ([Bacchelli and Bird, ICSE 2013](https://sback.it/publications/icse2013.pdf)).
...
Pass 4 adapts a subset of the writing rules in [ASD-STE100 Simplified Technical English](https://www.asd-ste100.org/) (Issue 9, January 2025). The 25-word ceiling is the standard's limit for descriptive text. The licensed dictionary half is not part of this. Vocabulary stays a judgment call.
